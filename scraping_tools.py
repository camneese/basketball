...
class PropositionLog:
    ...
    def __init__(self, player_list=[], stat_list=[], line_list=[]):
        
        # prop_df contains the names, statistics, and lines for the propositions.
        # player_object_list contains the cooresponding Player objects passed in.
        self.prop_df, self.player_object_list = self.make_prop_log(player_list=player_list, stat_list=stat_list, line_list=line_list)
    ...
    def make_prop_log(self, player_list=[], stat_list=[], line_list=[]):
        ...
        if len(player_list) == len(stat_list) and len(player_list) == len(line_list):
            ...
            return pd.DataFrame(prop_dict), player_list
        else:
            print('Mismatched lengths in the arguments provided. Unable to initialize a dataframe.')
            return pd.DataFrame(columns=['Player Name', 'Statistic', 'Line']), []
    ...
    def add_prop(self, player, stat, line):
        if stat in stats_dict.keys():
            ...
            print('The statistic you specified is not available. Call stats_dict.keys() for a list of available statistics.')
